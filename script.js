document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ── Marguerite active selon section ── */
  const sectionMap = {
  ...
  const targets = [
    ...
    ...document.querySelectorAll('.rejoindre-card'),
  ...
  document.querySelectorAll('.rejoindre-card').forEach((c,i)   => c.style.transitionDelay = `${i*0.12}s`);
  const obs = new IntersectionObserver(entries => {
    entries.forEach(e => { if (e.isIntersecting) { e.target.classList.add('visible'); obs.unobserve(e.target); } });
  }, { threshold: 0.10 });
  targets.forEach(el => obs.observe(el));

  /* ── Sway animation sur les marguerites ── */
  document.querySelectorAll('.nav-daisy-svg').forEach((svg, i) => {
    svg.style.animation      = `sway ${2.2 + i*0.22}s ease-in-out infinite`;
    svg.style.animationDelay = `${i * 0.14}s`;
  });
  ...
  btnResa?.addEventListener('click', () => {
    ...
    if (ok) {
      ...
      btnResa.style.background = '#4ade80'; btnResa.style.color = '#071F49';
